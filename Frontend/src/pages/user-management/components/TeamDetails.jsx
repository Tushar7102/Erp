...
const TeamDetails = ({ team, isOpen, onClose, onEdit, onDelete }) => {
  ...
  const fetchAllUsers = async () => {
    try {
      ...
      if (response.success) {
        ...
        if (teamLead) {
          if (teamLead.user_id) existingMemberIds.add(teamLead.user_id);
        ...
        const availableUsers = allUsersFromAPI.filter(user => {
          ...
          const isTusharPatil = 
            (user.first_name === 'Tushar' && user.last_name === 'Patil') || 
            user._id === '68ce363dccd03d0be2ab1d31';
          
          // Debug log to see what's happening with each user
          console.log(`User ${user.first_name} ${user.last_name} (${user._id}): isTeamMember=${isAlreadyTeamMember}, hasTeam=${hasTeamAssigned}, isTushar=${isTusharPatil}`);
          
          // Only include users who are not team members, don't have this team, and are not Tushar Patil
          return !isAlreadyTeamMember && !hasTeamAssigned && !isTusharPatil;
        });
  ...
  const handleUserSelect = async (userId) => {
    try {
      ...
      if (response.success) {
        // Also update the user's profile to assign this team
        try {
          const userResponse = await userService.getUser(userId);
          if (userResponse.success && userResponse.data) {
            const userData = userResponse.data;
          ...
          console.error('Error updating user profile with team:', userError);
              ...
              {activeTab === 'members' && (
                <div className="space-y-6">
                  ...
                  <div className="bg-white dark:bg-gray-700 shadow rounded-lg">
                    ...
                    {/* User Dropdown */}
                    {showUserDropdown && (
                      <div className="mx-6 mt-4 mb-2 p-4 bg-white dark:bg-gray-800 rounded-lg shadow border border-gray-200 dark:border-gray-700">
                        <div className="mb-3">
                            ...
                            {allUsers.length > 0 ? (
                              <ul className="divide-y divide-gray-200 dark:divide-gray-700">
                                {allUsers.map(user => (
                                  <li 
                                    key={user._id} 
                                    className={`p-2 cursor-pointer hover:bg-gray-100 dark:hover:bg-gray-700 ${selectedUserId === user._id ? 'bg-blue-50 dark:bg-blue-900/20' : ''}`}
                                    onClick={() => setSelectedUserId(user._id)}
                                  >
                                    <div className="flex items-center">
                                      {user.profile_image ? (
                                        <img src={user.profile_image} alt={user.name} className="w-8 h-8 rounded-full mr-2" />
                                      ) : (
                              ...
                              <p className="p-3 text-sm text-gray-500 dark:text-gray-400">
                                {allUsers.length === 0 ? "No available users to add" : "Loading users..."}
                              </p>
                            )}
                          </div>
                        </div>
                        ...
                        <div className="flex justify-end space-x-2">
                          ...
                          <button
                            className="px-4 py-2 text-sm font-medium text-white bg-blue-600 hover:bg-blue-700 dark:bg-blue-500 dark:hover:bg-blue-400 rounded-md disabled:opacity-50 disabled:cursor-not-allowed"
